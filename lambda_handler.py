...
def handle_config_request(headers: dict, query_params: dict) -> Dict[str, Any]:
    ...
    # Determine the best API_BASE_URL:
    # 1. Use VITE_API_BASE_URL env var if set
    # 2. Otherwise, derive from the request's own origin (custom domain support)
    api_base_url = os.environ.get("VITE_API_BASE_URL", "")
    if not api_base_url or api_base_url == "http://127.0.0.1:8000":
        # Derive from request headers — the frontend and backend share the same origin
        proto = headers.get("x-forwarded-proto", "https")
        host = (
